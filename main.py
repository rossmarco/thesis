...
class TextAnalyzer:
    ...
    def get_bigrams(self, size):
        ...
        sortedBiGrams = sorted(finder.ngram_fd.items(),
                               key=lambda t: (-t[1], t[0]))[:size]
        ...
        for bigram_tuple, count in sortedBiGrams:
            ...
            file_bigrams.writerow([type(bigram_tuple)(x.encode(
                'utf-8') for x in bigram_tuple), count])  # formatted properly #x.encode
    ...
    def get_trigrams(self, size):
        ...
        sortedTriGrams = sorted(finder.ngram_fd.items(
        ), key=lambda t: (-t[1], t[0]))[:size]  # doctest: +NORMALIZE_WHITESPACE

        # Store results of 400 bigrams into CSV file
        for trigram_tuple, count in sortedTriGrams:
            file_trigrams.writerow([type(trigram_tuple)(x.encode(
                'utf-8') for x in trigram_tuple), count])  # formatted properly x.encode
    ...
    def get_quadgrams(self, size):
        ...
        True
        sortedQuadGrams = sorted(finder.ngram_fd.items(
        ), key=lambda t: (-t[1], t[0]))[:size]  # doctest: +NORMALIZE_WHITESPACE

        # Store results of 400 bigrams into CSV file
        for quadgram_tuple, count in sortedQuadGrams:
            file_quadgrams.writerow([type(quadgram_tuple)(x.encode(
                'utf-8') for x in quadgram_tuple), count])  # formatted properly #x.encode
    ...
    def compare_csv(self, training_data, test_data, disease_type):
        ...
        with open(test_data, 'rb') as test_file:
            ...
            with open(file_name_results, 'wb') as results:
                ...
                similarity_metric = (
                    (num_of_matches/total_num_of_compares) * 100)
                ...
                message2 = 'The similarity metric is {0:.2f}'.format(
                    similarity_metric)
    ...
    def determine_best_match(self, smetric_cancer, smetric_diabetes, smetric_cvd, test_article_name, disease, ngram_size):
        ...
        if ngram_size == 1:
            ...
            raise ValueError(
                "Please only choose an n-gram size between 1-4, inclusive")
        ...
        if (self.smetric_cancer > self.smetric_diabetes) and (self.smetric_cancer > self.smetric_cvd):
            matches_writer.writerow(['The article ' + test_article_name +
                                     ' is a cancer article with a similary metric of {0:.2f}'.format(smetric_cancer)])
            if (disease == 'cancer'):
                correct_matches += 1
            else:
                incorrect_matches += 1
            ...
            matches_writer.writerow(['The article ' + test_article_name +
                                     ' is a diabetes article with a similary metric of {0:.2f}'.format(smetric_diabetes)])
            if (disease == 'diabetes'):
                correct_matches += 1
            else:
                incorrect_matches += 1
            ...
            matches_writer.writerow(['The article ' + test_article_name +
                                     ' is a CVD article with a similary metric of {0:.2f}'.format(smetric_cvd)])
            if (disease == 'cvd'):
                correct_matches += 1
            else:
                incorrect_matches += 1
            ...
            matches_writer.writerow(
                [test_article_name + ' unable to determine article type because two or more similarity metrics are equal'])
        else:
            matches_writer.writerow(
                [test_article_name + ' unable to determine article type because ?????????'])
...
def pdf_to_text(disease):
    ...
    files = [f for f in listdir(
        mypath) if ".py" not in f and "training" not in f and ".pdf" in f and isfile(join(mypath, f))]
...
def combine_text(disease):
    ...
    filenames = [f for f in listdir(
        mypath) if ".py" not in f and "training" not in f and ".pdf" not in f and isfile(join(mypath, f))]
...
def run_testing_data(disease, ngramsize, num_of_ngrams):
    ...
    files = [f for f in listdir(test_directory) if isfile and (f != '.DS_Store') and (
        'training' not in f) and ('.py' not in f) and (f != '.txt')]
    ...
    test_files = [t for t in listdir(test_directory) if (t != '.DS_Store') and (
        ".csv" in t) and ("training" not in t) and ('.py' not in f) and (f != '.txt')]

    for t in test_files:
        test_article.compare_csv(
            'cancer-training' + suffix + str(num_of_ngrams) + '.csv', t, 'cancer')
        test_article.compare_csv(
            'diabetes-training' + suffix + str(num_of_ngrams) + '.csv', t, 'diabetes')
        test_article.compare_csv(
            'cvd-training' + suffix + str(num_of_ngrams) + '.csv', t, 'cvd')
        test_article.determine_best_match(
            test_article.smetric_cancer, test_article.smetric_diabetes, test_article.smetric_cvd, str(t), disease, ngramsize)
